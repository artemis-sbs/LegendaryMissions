...
| Time | 30 |
| Build at | engineering |
| Program | kind=sensor, beacon_range=long |

A wider-reaching sensor relay: greater range costs more materials (2x salvage) and build time. The beacon's program carries beacon_range=long for the sensor sweep to read. (The key is beacon_range, not range: a recipe key can end up bound as a MAST variable, and \`range\` is one of MAST's own globals.)
